...
#################################
#---------------
myModel = torch.load("/content/Semantic_Human_Matting-fast_human_matting/ckpt/human_matting/model/model_obj.pth", map_location=device)
myModel.eval()
myModel.to(device)
...
def seg_processfg(image, net):
    ...
    image_resize = cv2.resize(image, (320,320), interpolation=cv2.INTER_CUBIC)
    image_resize = (image_resize - (104., 112., 121.,)) / 255.0



    tensor_4D = torch.FloatTensor(1, 3,320 , 320)
